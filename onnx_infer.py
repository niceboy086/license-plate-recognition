...
def allFilePath(rootPath,allFIleList):  #遍历文件
    ...
    for temp in fileList:
        ...
        if os.path.isfile(os.path.join(rootPath,temp)):
            ...
            #print(os.path.join(rootPath,temp))
        else:
...
def draw_result(orgimg,dict_list):
    #print("*debug, xxx*, orgimg:", orgimg.shape)
    result_str =""
    for result in dict_list:
        rect_area = result['rect']
        
        x,y,w,h = rect_area[0],rect_area[1],rect_area[2]-rect_area[0],rect_area[3]-rect_area[1]
        #print("*debug, xxx*, rect_area:", x,y,w,h)
        #print(orgimg.shape[1])
        padding_w = 0.05*w
        padding_h = 0.11*h
        rect_area[0]=max(0,int(x-padding_w))
        rect_area[1]=min(orgimg.shape[0],int(y-padding_h))
        ...
        result_str+=result+" "+color+" "
        for i in range(4):  #关键点
            cv2.circle(orgimg, (int(landmarks[i][0]), int(landmarks[i][1])), 5, clors[i], -1)
        cv2.rectangle(orgimg,(rect_area[0],rect_area[1]),(rect_area[2],rect_area[3]),(255,255,0),2) #画框
        if len(result)>=1:
            orgimg=cv2ImgAddText(orgimg,result+" "+color,rect_area[0]-height_area,rect_area[1]-height_area-10,(0,255,0),height_area)
    print(result_str)
    return orgimg

def det_rec_plate(session_det, session_rec, img_path):
    ...
    # print(img.shape)
    # output_names = session_detect.get_outputs()
    # for node in output_names:
    #     print(node.name)
    
    y_onnx = session_det.run([session_det.get_outputs()[0].name], {session_det.get_inputs()[0].name: img})[0]
...
if __name__ == "__main__":
    ...
    for pic_ in file_list:
        ...
        # outputs = post_precessing(y_onnx,r,left,top) #检测后处理
        # result_list=rec_plate(outputs,img0,session_rec)
        result_list, img0 = det_rec_plate(session_detect, session_rec, pic_)
